...
double neighboursCells(struct_matrix gameLand, int i, int j) {
	double n[gameLand.x][gameLand.y];
	for(i=0; i<gameLand.x; i++)
	{
		for(j=0; j<gameLand.y; j++)
		{
...
// KILLIAN - dont think this is set up correctly, how does the code know for the main loop what n is???
...
//				printf("-->%d - %d <--\n", i, j);
				...
				if (newGameLand.map[i][j].area == LAND) {
					...
					newGameLand.map[i][j].pumas = gameLand.map[i][j].pumas + dt * ( b * gameLand.map[i][j].hares * \
														 ...
														 gameLand.map[i][j-1].pumas + gameLand.map[i][j+1].pumas - \
														 n*gameLand.map[i][j].pumas));
				}
			}
		}
...
//	  printf("------------------------------------------------------------\n");
		if(fmod(t,10)==0)//something not working with fmod, keeps printing too many times
		{
